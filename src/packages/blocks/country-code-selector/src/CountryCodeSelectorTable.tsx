...
class CountryCodeSelectorTable extends BlockComponent<Props, S, SS> {
  ...
  actionOnRow(item: any) {
    ...
    setTimeout(function () {
      ...
      var countryNameCode = ` ${item.attributes.emoji_flag} ${item.attributes.name} (${item.id}) +${item.attributes.country_code}`;
    ...
    return (
      <View style={{ flex: 1 }}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <ListItem onPress={() => this.actionOnRow(item)}>
              <ListItem.Content>
                <ListItem.Title>
                  {`${item.attributes.emoji_flag} ${item.attributes.name} (${item.id}) +${item.attributes.country_code}`}
                </ListItem.Title>
              </ListItem.Content>
            </ListItem>
          )}
